...
| S7 | Zigbee backend, device swap, hybrid legs | Phase 2. The Zigbee backend landed in Phase 2A: pure protocol, adapter, managed groups, curated entries and the mixed-backend loop test. Not deployed and not built at setup (T42). Device swap and hybrid legs remain |
...
| T39 | The rule editor always targets the whole device, never one endpoint | `RuleTargetData.endpoint` exists and the compiler honours it (downgrading with a warning when the control cannot address an endpoint), and the editor sends null for every target. Every device on this network is single-endpoint, so nothing is lost here; a multi-endpoint target elsewhere would be linked whole. Closing it means an endpoint picker in the targets step, which needs the target device's endpoint list, which `devices/get` does not carry yet. |
...
| T42 | The Zigbee backend is not built at setup, so nothing in the product uses it yet | `_async_build_backends` builds Z-Wave only. Wiring Zigbee in needs an `MqttClient` over Home Assistant's `mqtt` integration, a base topic in the options flow (E25: a second Zigbee2MQTT instance uses a different one), and the Health sensor, diagnostics and config flow taught about a second backend. The Phase 2A plan does not ask for it and deployment is paused, so the whole backend is exercised through its tests and through the mixed-backend loop test rather than through a config entry. |
...
| T48 | A two-way Zigbee rule compiles a reverse leg that can never be written | `compiler._compile_reverse` writes the reverse leg from `writer_endpoint=0` and targets `rule.source.endpoint or None`, which are Z-Wave shapes: endpoint 0 is the Z-Wave root and does not exist on a Zigbee device, where the paddle is endpoint 2. The link is refused at apply time with `zigbee_source_cannot_send` and the compiler reports neither a warning nor an error, so the plan looks clean and never converges. The panel's rule editor defaults the `virtual_3way` template to two-way, so this is the canonical two-Inovelli-Blue case rather than an exotic one. Closing it means the compiler knowing which endpoint a target's control drives from, and `Emitter` deliberately has no endpoint (that lives on `zigbee_protocol.Control`), so it is a change to the shared capability model rather than something the adapter can do. Until it is made, `tests/test_zigbee_loop.py` pins the behaviour so it cannot be forgotten. |
| T49 | `JobRunner._is_system` refuses a whole endpoint cluster because one binding in it is the bridge's | "This slot holds a system entry, so the slot is system" is a Z-Wave truth (a lifeline group holds the controller and nothing else may go in it) and is false on Zigbee, where one endpoint's cluster holds many independent bindings side by side. A Zigbee device whose reporting binding sits on the same endpoint and cluster a rule uses, which is the usual shape for a battery remote or button, has every rule from it refused with no way out from the UI. Nothing in the G1 capture is shaped that way: the Inovelli reporting bindings are on endpoint 1 and on endpoint 2's manufacturer cluster, so no device on this network is affected. Phase 2A narrowed the guard from "this device's group" to "this endpoint's group", which is what made the ordinary Zigbee case work; closing the rest means deciding what the guard is for, which changes what `is_system` means to every backend. Pinned by a test. |
