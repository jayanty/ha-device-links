...
| T57 | The panel cannot author a rule whose source is a Zigbee device | Every device-addressing path in the panel names a device by its Home Assistant device id: `devices/get` and `devices/refresh` take one, the plan scope takes a list of them, the Devices view disables a row without one, and the rule editor's source picker filters candidates on `device_id !== null`. `rule_entity._upstream_identifier` derives an identifier for Z-Wave only (T13), so every Zigbee handle serializes with `device_id: null` and no Zigbee device can be opened, refreshed or chosen as a rule's source from the panel. They can be chosen as targets, so a Z-Wave control driving a Zigbee light is authorable today and the reverse is not. Found while closing T50, which is why the Zigbee half of `tests/test_panel_save_path.py` reaches `websocket._device_detail` directly rather than through `devices/get`: the payloads are the ones the panel would be handed, and the lookup in front of them is the part that does not work yet. Closing it means either an identity-addressed form of those commands or a Zigbee2MQTT identifier in `_upstream_identifier`, and the second is the better shape if Zigbee2MQTT's own MQTT discovery entries can be matched reliably, which nobody has checked. |
