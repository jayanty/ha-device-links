...
| T23 | E18 brings the entry up read-only, and today it refuses to load | `async_setup_entry` turns a `StorageSchemaError` into a translated `ConfigEntryError`, which leaves the unreadable file untouched (the half that cannot be added later) but does not give the user the read-only integration plus Repairs issue E18 asks for. Task 6 of Phase 1D owns Repairs and is where the rest lands. |
| T24 | Entity name strings were written ahead of Task 7 | `strings.json`, `translations/en.json` and `icons.json` carry the entity name and icon keys for the platforms built in Tasks 1 to 3, because an entity with an unresolved name translation has no name and therefore no stable entity id, so the tests could not be written without them. Everything else Task 7 owns (diagnostics keys, exceptions, services, config and options) is untouched, including the `no_backend_loaded`, `storage_unreadable` and `storage_no_migration` exception keys this phase raises with. |
