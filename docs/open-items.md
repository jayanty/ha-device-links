...
| A6 | Zigbee2MQTT republishes `bridge/devices` after a bind, and quickly enough for a verify to read it (part of J2, unproven) | A deep read after an apply waits five seconds for the republish and reports `unconfirmed` rather than `applied` if it does not come. If a real bridge is slower or does not republish at all, every Zigbee apply reads as unconfirmed and the deep-verify wait needs replacing with something else (there is no request that re-reads a binding table on demand) |
