...
| T50 | The rule editor names no target endpoint, so a panel-authored Zigbee rule is refused outright | T39 says the editor sends null for every target's endpoint, and adds that nothing is lost because every device on this network is single-endpoint. That was a statement about Z-Wave. A Zigbee binding always names a target endpoint, so a rule authored in the panel against an Inovelli Blue compiles a link with no target endpoint and every one of its links is refused at apply with `zigbee_target_endpoint_required`. Nothing is written and the plan cannot converge. The compiler now knows where a device receives (`DeviceCapabilities.receiving_endpoint`, added for T48), so the cheap fix is to use it when the rule names none, and the honest one is the endpoint picker T39 asks for; the difference is whether the user is told which endpoint was chosen for them. Not fixed with T48 deliberately: the reverse leg of a two-way rule has no field for the user to fill, and a forward target has one the editor chooses not to fill, and those are different problems. **This blocks the panel for Zigbee**, so it is the first thing Phase 2B should settle. |
